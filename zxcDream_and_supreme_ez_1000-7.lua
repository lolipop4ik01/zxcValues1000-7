-- ============================================
-- MM2 ULTIMATE CHECKER (ЧИСТАЯ ВЕРСИЯ)
-- НЕ ИСПОЛЬЗУЕТ ФУНКЦИИ EXECUTOR
-- ============================================
...
local function loadDataFromGitHub()
    ...
    print("Загружено Supreme: " .. tostring(#prices))
    print("Загружено DreamPets: " .. tostring(#dreampets))
    return true
end
...
local function updateAll()
    ...
    for i = 1, 4 do
        ...
        if slot then
            ...
            if name then
                ...
                local totalSupreme = math.floor(sp * amount)
                local totalDream = math.floor(dp * amount)
                
                local short = name
                if #short > 14 then short = short:sub(1,12)..".." end
                
                s.nameLabel.Text = short
                if totalDream > 0 then
                    s.priceLabel.Text = totalSupreme.." V\n"..totalDream.." ₽"
                else
                    s.priceLabel.Text = totalSupreme.." V"
                end
    ...
    for i = 1, 4 do
        ...
        if slot then
            ...
            if name then
                ...
                s.detailsLabel.Text = formatDetails(name, theirChromaMode[i])
            else
                s.nameLabel.Text = "Пусто"
                s.priceLabel.Text = "0 V"
                s.detailsLabel.Text = "❌"
    ...
    if yR > 0 then yourTotalDreamLabel.Text = "💸 "..math.floor(yR).." ₽"
    else yourTotalDreamLabel.Text = "" end
    
    if tR > 0 then theirTotalDreamLabel.Text = "💸 "..math.floor(tR).." ₽"
    else theirTotalDreamLabel.Text = "" end
